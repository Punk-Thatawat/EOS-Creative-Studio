...
export async function GET(_request: Request, context: { params: Promise<{ slug: string }> }): Promise<NextResponse> {
  try {
    ...
    const response = await fetch(`${backendApiUrl}/templates/${encodeURIComponent(slug)}`, { cache: "no-store", signal: AbortSignal.timeout(15000) });
    ...
    return NextResponse.json({ message: "Template service is unavailable" }, { status: 503 });
  }
}
