...
import { getAdminBillingSettings, getAdminTopups, saveAdminBillingSettings, saveAdminTopups, type TopupPackage } from '@/lib/api/credit-pricing';
...
export function TopupPackages() {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([getAdminTopups(), getAdminBillingSettings()]).then(([data, settings]) => { if (!active) return; setRows(data); setVatPercent(String(settings.vatPercent)); }).catch(e => { if (active) setError(e instanceof Error ? e.message : 'โหลดข้อมูลราคาไม่สำเร็จ'); }).finally(() => { if (active) { setLoading(false); setSettingsLoading(false); } });
    return () => { active = false; };
  }, []);
  ...
  return <section className="mb-6 rounded-2xl border border-orange-100 bg-white p-5" data-no-translate>
    ...
    <form className="mt-4 rounded-xl border border-orange-100 bg-orange-50/40 p-4" onSubmit={async e => {
      e.preventDefault(); if (!vatValid) { setVatError('VAT ต้องอยู่ระหว่าง 0–100%'); return; }
      setVatSaving(true); setVatError(''); setVatMessage('');
      try { const saved = await saveAdminBillingSettings({ vatPercent: Math.round(parsedVat * 100) / 100 }); setVatPercent(String(saved.vatPercent)); setVatMessage('บันทึก VAT แล้ว'); }
      catch (reason) { setVatError(reason instanceof Error ? reason.message : 'บันทึก VAT ไม่สำเร็จ'); }
      finally { setVatSaving(false); }
    }}>
      <div className="flex flex-wrap items-end justify-between gap-3"><label className="text-sm font-semibold">VAT (%)<input required type="number" min={0} max={100} step="0.01" value={vatPercent} disabled={settingsLoading || vatSaving} onChange={e => { setVatPercent(e.target.value); setVatError(''); setVatMessage(''); }} className="mt-1 block h-10 w-36 rounded-lg border bg-white px-3" /></label><p className="text-sm text-muted-foreground">ลูกค้าจะชำระราคาก่อน VAT + VAT ตามค่านี้ เช่น 100 บาท → {vatValid ? calculateVat(100, parsedVat).totalThb : '—'} บาท</p><button disabled={settingsLoading || vatSaving || !vatValid} className="rounded-lg bg-orange-600 px-4 py-2 font-bold text-white disabled:opacity-40">{vatSaving ? 'กำลังบันทึก...' : 'บันทึก VAT'}</button></div>
      {vatError && <p role="alert" className="mt-3 text-sm text-red-600">{vatError}</p>}
      {vatMessage && <p role="status" className="mt-3 text-sm text-green-700">{vatMessage}</p>}
    </form>
    {loading ? <p role="status">กำลังโหลดแพ็กเกจ...</p> : <form onSubmit={async e => {
      ...
      <fieldset disabled={saving} className="mt-4 space-y-3">
        {rows.map(row => <div key={row.id} className="grid items-end gap-3 rounded-xl border p-3 sm:grid-cols-[2fr_1fr_1fr_1fr_auto_auto]">
          ...
          <label className="text-sm">ราคา (ก่อน VAT)<input required type="number" min={10} max={100000} step={1} value={row.amountThb || ''} onChange={e => edit(row.id, { amountThb: Number(e.target.value), credits: Number(e.target.value) * 10 })} className="mt-1 block h-10 w-full rounded-lg border px-3" /></label>
          <p className="py-2 text-sm font-bold text-orange-600">{(row.amountThb * 10).toLocaleString()} เครดิต</p>
          <p className="py-2 text-sm font-bold text-pink-600">รวม VAT {vatValid ? calculateVat(row.amountThb, parsedVat).totalThb.toLocaleString('th-TH', { maximumFractionDigits: 2 }) : '—'} บาท</p>
