...
function ModelDetailsDialog({ item, feature, routeFeature, backgroundMode, onClose, onSaveDisplayName, onSaveInputLimits, onSavePreview, onSavePostAudio }: { item: GenerationModelOption; feature: FeatureId; routeFeature: string; backgroundMode?: AiBackgroundMode; onClose: () => void; onSaveDisplayName: (model: string, provider: string, displayName: string) => Promise<void>; onSaveInputLimits: (model: string, provider: string, limits: ModelUploadConstraints) => Promise<void>; onSavePreview: (model: string, provider: string, preview: { previewUrl: string | null; previewStorageKey: string | null; previewThumbnailStorageKey: string | null; previewType: ModelPreviewType | null }, backgroundMode?: AiBackgroundMode) => Promise<void>; onSavePostAudio: (routeFeature: string, model: string, provider: string, options: ModelPostAudioOptions) => Promise<void> }) {
  ...
  const [previewThumbnailStorageKeyDraft, setPreviewThumbnailStorageKeyDraft] = useState(item.previewThumbnailStorageKey ?? "");
  ...
  const uploadPreview = async (file: File) => {
    ...
    try {
      ...
      const uploaded = await uploadAdminModelPreview(file, { createThumbnail: true });
      setPreviewUrlDraft(uploaded.previewUrl ?? ""); setPreviewStorageKeyDraft(uploaded.storageKey); setPreviewThumbnailStorageKeyDraft(uploaded.thumbnailStorageKey ?? ""); setPreviewTypeDraft(uploaded.mediaType);
  ...
  const savePreview = async () => {
    ...
    try {
      ...
      const previewThumbnailStorageKey = previewStorageKey ? previewThumbnailStorageKeyDraft.trim() || null : null;
      await onSavePreview(item.model, item.provider, { previewUrl: previewStorageKey ? null : previewUrl, previewStorageKey, previewThumbnailStorageKey, previewType: previewUrl || previewStorageKey ? previewTypeDraft : null }, backgroundMode);
...
function AudioProviderSettingsPanel({ initialFeature = "textToSpeech" }: { initialFeature?: AdminAudioFeature }) {
  ...
  const [voiceAudioUploadingKey, setVoiceAudioUploadingKey] = useState("");
  const [voiceAudioUploadIndex, setVoiceAudioUploadIndex] = useState<number | null>(null);
  ...
  const voiceAudioInputRef = useRef<HTMLInputElement | null>(null);
  ...
  const uploadVoiceAudio = async (index: number, file: File) => {
    if (!file.type.startsWith("audio/")) {
      setError("กรุณาเลือกไฟล์เสียงเท่านั้น");
      return;
    }
    const uploadKey = `${selectedFeature}:${selectedFeatureModelId}:${index}`;
    setVoiceAudioUploadingKey(uploadKey);
    setMessage("");
    setError("");
    try {
      const uploaded = await uploadAdminAudioVoicePreview({ feature: selectedFeature, modelId: selectedFeatureModelId, voiceId: activeVoices[index]?.id ?? "", file });
      updateActiveVoice(index, { previewUrl: uploaded.previewUrl, previewStorageKey: uploaded.previewStorageKey });
      setMessage("อัปโหลดเสียงตัวอย่างแล้ว — กด Save provider settings เพื่อบันทึก");
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Unable to upload voice preview");
    } finally {
      setVoiceAudioUploadingKey("");
    }
  };
...
function AdminModelRoutesContent() {
  ...
  const saveModelPreview = async (model: string, provider: string, preview: { previewUrl: string | null; previewStorageKey: string | null; previewThumbnailStorageKey: string | null; previewType: ModelPreviewType | null }, previewBackgroundMode?: AiBackgroundMode) => {
    setBusy(true);
    setError("");
    try {
      ...
      <div className="mx-auto max-w-[1180px] pt-6 lg:pt-8">
        ...
        {feature === "audio" || feature === "image-to-video" ? null : <div className="grid gap-6 lg:grid-cols-1 lg:items-start">
          <section aria-labelledby="route-heading" className="min-w-0">
            <div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary">Route configuration</p><h2 id="route-heading" className="mt-1 text-xl font-bold tracking-tight">{activeFeature.label}</h2></div><SearchInput size="compact" className="w-full sm:w-60" value={query} onValueChange={setQuery} placeholder="Search models" aria-label="Search models" /></div>
