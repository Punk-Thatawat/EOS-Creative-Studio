...
import { AlertCircle, CheckCircle2, ChevronLeft, ChevronRight, Coins, LoaderCircle, Plus, RefreshCw, ShieldCheck, UserPlus, UserRound, UsersRound, X } from "lucide-react";
...
import { grantAdminMemberCredits, inviteAdminMember, listAdminMembers, updateAdminMember, type AdminMember, type AdminMemberRole, type AdminMemberStatus } from "@/lib/api/admin-members";
...
function AdminMembersContent() {
  ...
  return <SidebarProvider><div className="min-h-screen w-full min-w-0 bg-background"><SidebarNavigation /><div className="min-w-0 xl:pl-[var(--sidebar-width)]"><StudioHeader /><main className="page-gutter mx-auto w-full max-w-[1600px] py-5 lg:py-8"><div className="min-h-[calc(100vh-120px)] overflow-x-clip rounded-3xl bg-[#faf8f6] px-4 pb-20 sm:px-6 lg:px-8 lg:pb-24"><div className="mx-auto max-w-[1180px] pt-6 lg:pt-8">
    <div className="mb-8 flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[#201d1b] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.14em] text-white"><UsersRound size={13} /> Operations</div><h1 className="max-w-xl text-3xl font-bold tracking-tight sm:text-4xl">Members & roles</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Manage account access, role permissions, and member credit balances from one place.</p></div><div className="flex flex-wrap gap-2"><Button size="lg" onClick={() => { setError(""); setMessage(""); setInviteOpen(true); }}><UserPlus size={16} /> Invite member</Button><Button variant="outline" size="lg" onClick={() => void load()} disabled={loading || Boolean(busyId)}><RefreshCw size={16} className={loading ? "animate-spin" : undefined} /> Refresh</Button></div></div>
