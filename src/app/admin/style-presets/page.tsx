...
import { createAdminStylePreset, disableAdminStylePreset, listAdminStylePresets, stylePresetFeatureOptions, updateAdminStylePreset, type GenerationStylePreset, type SaveStylePresetInput, type StylePresetFeature } from "@/lib/api/style-presets";
...
function PresetEditor({ draft, busy, onChange, onSave, onClose }: { draft: Draft; busy: boolean; onChange: (next: Draft) => void; onSave: () => void; onClose: () => void }) {
...
function AdminStylePresetsContent() {
  ...
  const save = async () => {
    ...
    try {
      const input: SaveStylePresetInput = { slug: draft.slug, name: draft.name, prompt: draft.prompt, imageUrl: draft.imageUrl ?? "", features: draft.features, enabled: draft.enabled, sortOrder: draft.sortOrder };
