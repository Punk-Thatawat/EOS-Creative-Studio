...
export function RightRail() {
  ...
  useEffect(() => {
    ...
    void fetchHomeDashboard()
      .then((dashboard) => {
        if (mounted) setCredits(dashboard.credits);
      })
      .catch(() => undefined);
    return () => {
      mounted = false;
    ...
    <aside className="eos-studio-right-rail hidden w-[var(--rail-width)] shrink-0 space-y-4 xl:block">
      <Card className="overflow-hidden bg-[#201d1b] text-white">
        <div className="p-5">
          ...
          <p className="text-xs text-white/60">{text.rail.creditsRemaining}</p>
          <p className="mt-1 text-3xl font-bold">{credits ? format(credits.available) : "—"}</p>
          ...
          <div className="mt-2 flex justify-between text-[10px] text-white/50">
            <span>
              {credits ? `${usedPercent}%` : "—"} {text.rail.used}
            </span>
            <span>
              {credits ? format(credits.allowance) : "—"} {text.rail.total}
            </span>
          </div>
        </div>
        <Link
          href="/usage"
          className="flex items-center justify-between border-t border-white/10 px-5 py-3 text-xs font-bold text-[#ffb18e] hover:bg-white/5"
        >
          {text.rail.viewUsage} <ArrowUpRight size={14} />
        </Link>
      </Card>
      <Card className="p-5">
        <div className="flex items-center gap-2 text-sm font-bold">
          <Sparkles size={16} className="text-primary" /> {text.rail.quickTip}
        </div>
        <p className="mt-3 text-xs leading-5 text-muted-foreground">{text.rail.tip}</p>
        <Link href="/templates" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-primary">
          {text.rail.browseTemplates} <ArrowUpRight size={13} />
        </Link>
      </Card>
    </aside>
  );
