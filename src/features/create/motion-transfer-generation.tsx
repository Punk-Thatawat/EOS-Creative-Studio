    ...
    <div className={styles.columns}>
      ...
      <aside className={styles.settings}>
        ...
        <div className={styles.modelDropdown}>
          <button type="button" className={styles.modelDropdownTrigger} aria-haspopup="listbox" aria-expanded={isModelMenuOpen} disabled={modelsLoading || models.length === 0} onClick={() => setIsModelMenuOpen((open) => !open)}><span><strong>{modelsLoading ? "Loading motion transfer models…" : selectedModelOption?.displayName ?? "No motion transfer model"}</strong><span className={styles.modelProviderRow}><small>{selectedModelOption?.provider ?? ""}</small>{selectedModelOption ? <b className={`${styles.modelTierBadge} ${styles[modelTierClass(modelTier(selectedModelOption, Math.max(0, models.findIndex((item) => item.model === selectedModel))))]}`}>{modelTier(selectedModelOption, Math.max(0, models.findIndex((item) => item.model === selectedModel)))}</b> : null}</span></span><ChevronDown size={17} /></button>
