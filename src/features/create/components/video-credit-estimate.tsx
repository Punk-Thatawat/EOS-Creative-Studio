...
export function VideoCreditEstimate({ featureLabel, duration, estimate, emptyMessage = "Pricing unavailable", emptyLoading = false, compactLabel = false }: { featureLabel: string; duration?: unknown; estimate: ReturnType<typeof useVideoCreditEstimate>; emptyMessage?: string; emptyLoading?: boolean; compactLabel?: boolean }) {
  ...
  return <div className={styles.estimate} title={estimate.error ?? undefined}><div>ESTIMATED CREDITS <Info size={11} /></div><span>{quantityLabel}<strong>{value}</strong></span></div>;
}
