...
export function ExtendPanel({
  ...
  const { t } = useLocale();
  return (
    <aside className={cx("gen-panel", "gen-prompt-panel", "gen-extend-panel")}>
      <div className={cx("gen-prompt-top-action")}>
        <span>{tutorialButton}</span>
        <span>{clearButton}</span>
      </div>
      <div className={cx("gen-panel-title")}>
        <h2>
          {t("create.prompt")} <em>({t("create.optional")})</em>
        </h2>
        <Image
          src="/generated-assets/be-descriptive.png"
          alt={t("create.image.beDescriptive")}
          width={2051}
          height={509}
          className={cx("gen-prompt-annotation")}
        />
      </div>
      <PromptField
        id="gen-extend-prompt"
        value={prompt}
        onChange={onPromptChange}
        placeholder={t("create.image.promptHint.extend")}
        ariaLabel={t("create.image.extend.promptAria")}
        wrapperClassName={cx("gen-textarea-wrap", "gen-extend-prompt")}
        metaClassName={cx("gen-prompt-meta")}
      />
      <PromptOptimizerToggle enabled={promptOptimizerEnabled} onChange={onPromptOptimizerChange} />
      <div className={cx("gen-section-heading")}>
        <h3>
          {t("create.image.sourceImage")} <em>({t("create.required")})</em>
        </h3>
      </div>
      <SourceImageUpload
        imageUrl={sourceImage}
        onImageChange={onSourceImageChange}
        onClear={onSourceImageClear}
        purpose="content"
      ...
      <div className={cx("gen-section-heading", "gen-inline-heading")}>
        <h3>{t("create.image.extend.direction")}</h3>
        <InfoTooltip content={t("create.settings.info.expandDirection")} size={12} />
      </div>
      <div
        className={cx("gen-extend-direction-grid")}
        role="radiogroup"
        aria-label={t("create.image.extend.direction")}
      >
        {extendDirections.map((item) => {
          ...
          return (
            <button
              ...
              className={cx(direction === item && "is-selected", item === "all" && "is-all")}
              onClick={() => onDirectionChange(item)}
            >
              <span className={cx("gen-extend-direction-icon")}>
                <Icon size={16} />
              </span>
              <span>
                <b>{label}</b>
      ...
      <div className={cx("gen-section-heading", "gen-inline-heading")}>
        <h3>{t("create.image.extend.amount")}</h3>
        <InfoTooltip content={t("create.settings.info.expandAmount")} size={12} />
      </div>
      <div
        className={cx("gen-segmented", "gen-extend-amounts")}
        role="radiogroup"
        aria-label={t("create.image.extend.amount")}
      >
        {extendAmounts.map((item) => (
          <button
            type="button"
            key={item}
            role="radio"
            aria-checked={amount === item}
            className={cx(amount === item && "is-selected")}
      ...
      <details className={cx("gen-advanced")}>
        <summary>
          <span>{t("create.image.advanced")}</span>
          <span aria-hidden="true">⌄</span>
        </summary>
        <div className={cx("gen-advanced-body")}>
          <div className={cx("gen-advanced-field")}>
            <label htmlFor="gen-negative-prompt-extend">{t("create.image.negativePrompt")}</label>
            <PromptField
              id="gen-negative-prompt-extend"
              value={negativePrompt}
              onChange={onNegativePromptChange}
              placeholder={t("create.image.extend.negativeHint")}
              ariaLabel={t("create.image.extend.negativeAria")}
              multiline={false}
              maxLength={100}
              wrapperClassName={cx("gen-input-wrap")}
              metaClassName={cx("gen-prompt-meta")}
            />
          </div>
        </div>
