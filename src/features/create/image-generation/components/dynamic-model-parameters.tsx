...
function DynamicField({
  ...
  if (Array.isArray(property.enum) && property.enum.length > 0) {
    return (
      <div className={cx("gen-dynamic-field")}>
        <label>{label}</label>
        <Dropdown
          value={value === undefined ? "" : String(value)}
          options={property.enum.map((option) => ({ value: String(option), label: String(option) }))}
          onChange={(nextValue) => onChange(parseValue(nextValue, property))}
          placeholder={t("create.image.params.auto")}
          ariaLabel={label}
          triggerClassName={cx("gen-select")}
          menuClassName={cx("gen-select-menu")}
        />
  ...
  if (type === "boolean") {
    return (
      <label className={cx("gen-dynamic-checkbox")}>
  ...
  if (type === "array" || type === "object") {
    ...
    return (
      <div className={cx("gen-dynamic-field")}>
        <label htmlFor={`model-param-${name}`}>
          {label}
          <small>JSON</small>
        </label>
        <textarea
          id={`model-param-${name}`}
          value={serialized}
          placeholder={type === "array" ? '["value"]' : '{"key": "value"}'}
          onChange={(event) => {
            try {
              onChange(event.target.value.trim() ? JSON.parse(event.target.value) : undefined);
            } catch {
              /* Keep the current valid value until JSON is complete. */
            }
          }}
        />
        {description && <small>{description}</small>}
  ...
  return (
    <div className={cx("gen-dynamic-field")}>
      <label htmlFor={`model-param-${name}`}>{label}</label>
      <input
        id={`model-param-${name}`}
        type={
        ...
        placeholder={property.default === undefined ? "Auto" : String(property.default)}
        onChange={(event) => onChange(parseValue(event.target.value, property))}
      />
      {description && <small>{description}</small>}
    </div>
  );
...
export function DynamicModelParameters({ capabilities, values, onChange }: DynamicModelParametersProps) {
  ...
  const fields = useMemo(
    () => Object.entries(properties ?? {}).filter(([name, property]) => isRenderableProperty(name, property)),
    [properties],
  );
  ...
  return (
    <details
      className={cx("gen-advanced", "gen-dynamic-params")}
      open={open}
      onToggle={(event) => setOpen(event.currentTarget.open)}
    >
      <summary>
        <span>
          <SlidersHorizontal size={13} /> {t("create.image.params.title")} <small>{fields.length} model-specific</small>
        </span>
        <ChevronDown size={14} />
      </summary>
      <div className={cx("gen-advanced-body", "gen-dynamic-params-body")}>
        ...
        <p className={cx("gen-dynamic-note")}>{t("create.image.params.note")}</p>
      </div>
    </details>
  );
}
