...
function PreviewSurface({ image, placeholderImage = null, alt, placeholderLabel, isLoading = false, generated = false, showLiveBadge = true, actionAvailable = Boolean(image), isFavorite, onDownload, onToggleFavorite, onOpenImage, onImageError, onImageLoad, showActions = true, className, backgroundColor, style, children }: { image: string | null; placeholderImage?: string | null; alt: string; placeholderLabel: string; isLoading?: boolean; generated?: boolean; showLiveBadge?: boolean; actionAvailable?: boolean; isFavorite: boolean; onDownload: () => void; onToggleFavorite: () => void; onOpenImage: () => void; onImageError?: () => void; onImageLoad?: () => void; showActions?: boolean; className?: string; backgroundColor?: string; style?: CSSProperties; children?: ReactNode }) {
  const { t } = useLocale();
  ...
  return <div className={cx("gen-preview-image", generated && "is-generated", isLoading && "is-preview-loading", className)} style={{ ...style, ...(backgroundColor ? { backgroundColor } : {}) }} aria-busy={isLoading}>
    {isLoading ? <ImagePlaceholder className={cx("is-loading")} label={placeholderLabel} /> : image ? <>
      {placeholderImage && <img src={placeholderImage} alt="" aria-hidden="true" decoding="async" className={cx("gen-model-preview-thumbnail", isImageLoaded && "is-loaded")} />}
      <img src={image} alt={alt} decoding="async" fetchPriority="high" className={cx("gen-generated-image", placeholderImage && "is-model-preview", placeholderImage && !isImageLoaded && "is-model-preview-loading")} onError={onImageError} onLoad={() => { setIsImageLoaded(true); onImageLoad?.(); }} />
...
function ExtendPreview({ sourceImage, resultImage, modelPreviewUrl, modelPreviewThumbnailUrl, aspectRatio, direction, amount, isFavorite, isLoading, placeholderLabel, onDownload, onToggleFavorite, onOpenImage, onImageError, onImageLoad }: { sourceImage: string | null; resultImage: string | null; modelPreviewUrl?: string | null; modelPreviewThumbnailUrl?: string | null; aspectRatio: string; direction: ExtendDirection; amount: ExtendAmount; isFavorite: boolean; isLoading: boolean; placeholderLabel: string; onDownload: () => void; onToggleFavorite: () => void; onOpenImage: () => void; onImageError: () => void; onImageLoad: () => void }) {
  const { t } = useLocale();
...
export function PreviewPanel({ activeTab, generated, generatedImageUrls, generationCompletedCount, generationStatus, generationTotalCount, isGenerating, isLoadingRecent, recentError, recentGenerationUrls, imageMimeTypes, selectedRecentImageUrl, selectedVariation, previewDisplayMode, onPreviewDisplayModeChange, sourceImage, previewRatio, modelPreviewUrl = null, modelPreviewThumbnailUrl = null, modelPreviewType = null, backgroundMask = null, backgroundMode = "remove", backgroundColor = "#ffffff", backgroundTransparent = true, maskTool, brushSize, extendDirection = "right", extendAmount = "50%", onBackgroundMaskChange, onMaskToolChange, onBrushSizeChange, onRecentSelect, onVariationSelect, onRefreshRecent }: { activeTab: ImageGenerationTab; generated: boolean; generatedImageUrls: string[]; generationCompletedCount: number; generationStatus: GenerationStatus; generationTotalCount: number; isGenerating: boolean; isLoadingRecent: boolean; recentError: string | null; recentGenerationUrls: string[]; imageMimeTypes: Record<string, string>; selectedRecentImageUrl: string | null; selectedVariation: number; previewDisplayMode: "current" | "gallery" | "model"; onPreviewDisplayModeChange: (mode: "current" | "gallery" | "model") => void; sourceImage: string | null; previewRatio: string; modelPreviewUrl?: string | null; modelPreviewThumbnailUrl?: string | null; modelPreviewType?: "image" | "video" | null; backgroundMask?: string | null; backgroundMode?: BackgroundMode; backgroundColor?: string; backgroundTransparent?: boolean; maskTool: MaskTool; brushSize: number; extendDirection?: ExtendDirection; extendAmount?: ExtendAmount; onBackgroundMaskChange: (mask: string | null) => void; onMaskToolChange: (tool: MaskTool) => void; onBrushSizeChange: (size: number) => void; onRecentSelect: (url: string) => void; onVariationSelect: (index: number) => void; onRefreshRecent: () => void }) {
  const { t } = useLocale();
