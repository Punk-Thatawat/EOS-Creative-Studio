...
*/
function ExtendPreview({ sourceImage, resultImage, direction, amount, isFavorite, isLoading, placeholderLabel, onDownload, onToggleFavorite, onOpenImage }: { sourceImage: string | null; resultImage: string | null; direction: ExtendDirection; amount: ExtendAmount; isFavorite: boolean; isLoading: boolean; placeholderLabel: string; onDownload: () => void; onToggleFavorite: () => void; onOpenImage: () => void }) {
  const hasResult = Boolean(resultImage);
  return <PreviewSurface image={hasResult ? resultImage : null} alt={hasResult ? "Extended image" : "Source image preview"} placeholderLabel={placeholderLabel} isLoading={isLoading} generated={hasResult} actionAvailable={hasResult} isFavorite={isFavorite} onDownload={onDownload} onToggleFavorite={onToggleFavorite} onOpenImage={onOpenImage} className={cx("gen-extend-preview")}>
    {!hasResult && sourceImage && !isLoading && <div className={cx("gen-extend-canvas", `is-${direction}`)}><div className={cx("gen-extend-zone")}><span>{direction === "all" ? "NEW AREA · ALL SIDES" : "NEW AREA"}</span><b>{direction === "all" ? `+${amount} EACH SIDE` : `+${amount}`}</b></div><img src={sourceImage} alt="Original image boundary" className={cx("gen-extend-source")} /><span className={cx("gen-extend-original-label")}>ORIGINAL</span><span className={cx("gen-extend-direction-label")}>{direction === "all" ? "AI will continue the scene in all four directions" : `AI will continue the scene ${direction}`}</span></div>}
  </PreviewSurface>;
}

export function PreviewPanel({ activeTab, generated, generatedImageUrls, generationCompletedCount, generationStatus, generationTotalCount, isGenerating, isLoadingRecent, recentError, recentGenerationUrls, imageMimeTypes, selectedRecentImageUrl, selectedVariation, previewDisplayMode, onPreviewDisplayModeChange, sourceImage, backgroundMask = null, backgroundMode = "remove", backgroundColor = "#ffffff", backgroundTransparent = true, maskTool, brushSize, extendDirection = "right", extendAmount = "50%", onBackgroundMaskChange, onMaskToolChange, onBrushSizeChange, onRecentSelect, onVariationSelect, onRefreshRecent }: { activeTab: ImageGenerationTab; generated: boolean; generatedImageUrls: string[]; generationCompletedCount: number; generationStatus: GenerationStatus; generationTotalCount: number; isGenerating: boolean; isLoadingRecent: boolean; recentError: string | null; recentGenerationUrls: string[]; imageMimeTypes: Record<string, string>; selectedRecentImageUrl: string | null; selectedVariation: number; previewDisplayMode: "current" | "gallery"; onPreviewDisplayModeChange: (mode: "current" | "gallery") => void; sourceImage: string | null; backgroundMask?: string | null; backgroundMode?: BackgroundMode; backgroundColor?: string; backgroundTransparent?: boolean; maskTool: MaskTool; brushSize: number; extendDirection?: ExtendDirection; extendAmount?: ExtendAmount; onBackgroundMaskChange: (mask: string | null) => void; onMaskToolChange: (tool: MaskTool) => void; onBrushSizeChange: (size: number) => void; onRecentSelect: (url: string) => void; onVariationSelect: (index: number) => void; onRefreshRecent: () => void }) {
  ...
  const genericPreviewImage = selectedImageUrl ?? ((activeTab === "Image to Image" || activeTab === "AI Style Transfer" || activeTab === "Upscale" || activeTab === "Extend Image") && sourceImage && !generated ? sourceImage : null);
  const genericPreviewLabel = previewStatus ?? (generated ? "GENERATION PREVIEW" : activeTab === "Image to Image" || activeTab === "AI Style Transfer" || activeTab === "Upscale" || activeTab === "Extend Image" ? "UPLOAD SOURCE IMAGE" : "PREVIEW IMAGE");
  const backgroundPreviewImage = effectiveBackgroundPreviewMode === "before" ? sourceImage : effectiveBackgroundPreviewMode === "mask" ? backgroundMask ?? sourceImage : selectedImageUrl ?? sourceImage;
  const backgroundPreviewLabel = effectiveBackgroundPreviewMode === "before" ? "Before background edit" : effectiveBackgroundPreviewMode === "mask" ? "Black and white subject removal mask" : selectedImageUrl ? "After background edit" : "Uploaded source image preview";
  ...
  const previewImageLabel = isGalleryView ? `${galleryKind === "variation" ? "Variation" : "Recent generation"} preview` : isBackgroundPreview ? backgroundPreviewLabel : selectedImageUrl ? "Generated image" : "Source image preview";
  ...
  return <section className={cx("gen-panel", "gen-preview-panel")}>
    ...
    {isExtendPreview ? <ExtendPreview sourceImage={sourceImage} resultImage={selectedImageUrl ?? null} direction={extendDirection} amount={extendAmount} isFavorite={isFavorite} isLoading={isPreviewLoading} placeholderLabel={isPreviewLoading ? (previewStatus ?? "CREATING PREVIEW") : genericPreviewLabel} onDownload={() => void downloadImage()} onToggleFavorite={() => setIsFavorite((favorite) => !favorite)} onOpenImage={openDisplayedImage} /> : <PreviewSurface image={previewImage} alt={previewImageLabel} placeholderLabel={previewPlaceholderLabel} isLoading={isPreviewLoading} generated={previewIsGenerated} actionAvailable={Boolean(displayedImageUrl) && !isBackgroundMaskEditing} isFavorite={isFavorite} onDownload={() => void downloadImage()} onToggleFavorite={() => setIsFavorite((favorite) => !favorite)} onOpenImage={openDisplayedImage} onImageError={() => previewImage && markImageUnavailable(previewImage)} showActions={!isBackgroundMaskEditing} className={cx(isBackgroundPreview && "is-background-preview", isBackgroundMaskEditing && "is-background-mask-editing")} backgroundColor={isBackgroundPreview && backgroundMode === "solid" && effectiveBackgroundPreviewMode === "after" && !isGalleryView ? backgroundColor : undefined}>
