...
export function ExtendVideoWorkspace() {
  ...
  const handleSourceVideoDrop = (event: React.DragEvent<HTMLElement>) => {
    event.preventDefault();
    setIsSourceVideoDragging(false);
    const file = event.dataTransfer.files?.[0];
    if (file) void setAsset(file, "video");
  };
  const handleAudioDrop = (event: React.DragEvent<HTMLElement>) => {
    event.preventDefault();
    setIsAudioDragging(false);
    const file = event.dataTransfer.files?.[0];
    if (file) void setAsset(file, "audio");
  };
  ...
  return <div className={styles.columns}>
    <div className={styles.leftColumn}>
      ...
      <div className={styles.centerColumn}><section className={`${styles.previewPanel} ${styles.videoPreviewPanel}`}><div className={styles.videoPreview}><VideoPreviewLiveBadge />{isGenerating ? <div className={styles.videoGeneratingPreview} aria-busy="true"><Plus size={26} /><strong>{state === "uploading" ? t("create.video.common.preparingVideo") : t("create.video.common.generatingVideo")}</strong><span>{notice ?? t("create.video.extend.generatingNotice")}</span><div className={styles.videoGenerationProgress}><i style={{ width: `${progress || 12}%` }} /></div><small>{progress ? t("create.video.common.percentComplete", { percent: progress }) : t("create.video.common.working")}</small></div> : previewView === "model" && selected?.previewUrl ? <ModelPreviewMedia url={selected.previewUrl} type={selected.previewType} autoPlay={selected.previewType === "video"} alt={`${selected.displayName} model preview`} className={`${styles.generatedVideoPlayer} ${styles.motionGeneratedVideoPlayer}`} frameClassName={styles.videoPreviewMediaFrame} /> : previewVideoUrlForView ? <EosVideoPlayer src={previewVideoUrlForView} className={`${styles.generatedVideoPlayer} ${styles.motionGeneratedVideoPlayer}`} mediaFrameClassName={styles.videoPreviewMediaFrame} ariaLabel={t("create.video.common.generatedVideo")} /> : <VideoPreviewPlaceholder showActions={false} />}{previewVideoUrlForView ? <VideoPreviewOverlayActions videoUrl={previewVideoUrlForView} /> : null}</div></section><VideoResultLibrary feature="extend-video" currentVideoUrl={finalVideoUrl} currentSourceGenerationId={generationId} selectedVideoUrl={previewVideoUrlForView} refreshKey={libraryRefreshKey} view={previewView} onViewChange={setPreviewView} onVideoSelect={(url, view) => { setPreviewVideoUrl(url); setPreviewView(view); }} /></div>
