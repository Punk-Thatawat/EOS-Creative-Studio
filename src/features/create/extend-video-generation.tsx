...
export function ExtendVideoWorkspace() {
  ...
  return <div className={styles.columns}>
    <div className={styles.leftColumn}>
      ...
      <section className={`${styles.panel} ${styles.videoPromptPanel}`}>
        ...
        <label className={styles.videoPromptInputLabel}>
          <textarea className={styles.videoPromptTextarea} value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Continue the camera movement naturally, keep the same subject, lighting, and style…" maxLength={4000} required aria-required="true" />
        </label>
        <div className={styles.videoPromptMeta}>
          <span>Maximum 4,000 characters</span>
        ...
        <label className={styles.peopleFieldLabel}>Negative Prompt <small>(Optional)</small><input value={negativePrompt} onChange={(event) => setNegativePrompt(event.target.value)} placeholder="blurry, jump cut, flicker, distorted subject" /></label>
      </section>
    ...
    <div className={styles.centerColumn}><section className={`${styles.previewPanel} ${styles.videoPreviewPanel}`}><div className={styles.videoPreview}><VideoPreviewLiveBadge />{isGenerating ? <div className={styles.videoGeneratingPreview} aria-busy="true"><Plus size={26} /><strong>{state === "uploading" ? "PREPARING VIDEO" : "EXTENDING VIDEO"}</strong><span>{notice ?? "Working…"}</span><div className={styles.videoGenerationProgress}><i style={{ width: `${progress || 12}%` }} /></div><small>{progress ? `${progress}% complete` : "Waiting for provider…"}</small></div> : displayedVideoUrl ? <EosVideoPlayer src={displayedVideoUrl} className={`${styles.generatedVideoPlayer} ${styles.motionGeneratedVideoPlayer}`} mediaFrameClassName={styles.videoPreviewMediaFrame} ariaLabel="Extended video preview" /> : selected?.previewUrl ? <ModelPreviewMedia url={selected.previewUrl} type={selected.previewType} alt={`${selected.displayName} model preview`} className={`${styles.generatedVideoPlayer} ${styles.motionGeneratedVideoPlayer}`} frameClassName={styles.videoPreviewMediaFrame} /> : <VideoPreviewPlaceholder />}{displayedVideoUrl ? <VideoPreviewOverlayActions videoUrl={displayedVideoUrl} /> : null}</div></section><VideoResultLibrary feature="extend-video" currentVideoUrl={finalVideoUrl} selectedVideoUrl={displayedVideoUrl} refreshKey={libraryRefreshKey} onVideoSelect={(url) => setPreviewVideoUrl(url)} /></div>
