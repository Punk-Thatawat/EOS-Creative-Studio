      ...
      {loginOpen && <div className="auth-modal" role="dialog" aria-modal="true" aria-label={authMode === "login" ? t("auth.modal.loginTitle") : authMode === "register" ? t("auth.modal.registerTitle") : authMode === "forgot" ? t("auth.modal.forgotTitle") : t("auth.modal.confirmationTitle")} onClick={closeLogin}>
        <div className="auth-modal-shell" onClick={(event) => event.stopPropagation()}>
          ...
          {authMode === "confirmation" ? <div className="auth-confirmation-state">
            ...
            {authMode !== "forgot" && <div className="auth-divider" style={{ display: "none" }}><span>{t("auth.action.continueWith")}</span></div>}
            {googleLoginError && <p className="auth-error" role="alert">{googleLoginError}</p>}
            {authMode !== "forgot" && <><div className="auth-socials"><button type="button" onClick={() => { void handleGoogleLogin(); }} disabled={googleLoginLoading || authSubmitting} aria-busy={googleLoginLoading} style={{ display: "none" }}><Image src="/generated-assets/google-g-icon.svg" alt="" width={18} height={18} /> <span>{googleLoginLoading ? t("auth.action.connecting") : "Google"}</span></button></div><p className="auth-signup" style={{ display: "none" }}>{authMode === "login" ? <>{t("auth.footer.noAccount")} <button type="button" onClick={() => switchAuthMode("register")}>{t("auth.action.signUp")}</button></> : <>{t("auth.footer.hasAccount")} <button type="button" onClick={() => switchAuthMode("login")}>{t("auth.action.login")}</button></>}</p></>}
          </>}
          </div>
        </div>
      </div>}
